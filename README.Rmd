...
#### Titles
By default `title` and `subtitle` and `caption` are set to `NULL`. In this case automatic titles will be used the `title` will display the name of the plotting function and the run number for example __DV vs. IPRED |run001__ the `subtitle` will show additional information such as EPS shrinkage for IPRED based plots and the `caption` will show the run directory. Templates can be provided via strings to generate informative `title`, `subtitle` and `caption`. To do so simply use one of the keywords preceded by `@` (e.g. `@ofv`) and the keyword will be automatically replaced by the value contained in the xpose_data object when generating the plot. 
