...
### Visual guides
The option `guides` can enable (`TRUE`) or disable (`FALSE`) the visual guides on the graphs such as unity line on the `dv_vs_ipred()` plot or horizontal lines on `cwres_vs_idv()`.
...
Themes can be attached to an `xpdb` when importing the data with `xpose_data()`, using the function `update_themes()` on an xpdb object or when creating a graph.
