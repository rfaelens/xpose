...
### Visual guides
The option `guides` can enable (`TRUE`) or disable (`FALSE`) the visual guides on the graphs such as unity line on the `dv_vs_ipred()` type plot or horizontal lines on residual plots (e.g. `res_vs_idv()`).
...
```
Note: suffixes from `xp_theme` will not be automatically added to the title, subtitle and caption when defined in the lab function.
...
* If `facets` is a string e.g `facets = "OCC"`, `ggplot2::facet_wrap` will be used
* If `facets` is a formula e.g `facets = SEX~OCC`, `ggplot2::facet_grid` will be used

```{r demo panels, fig.width = 6, out.width = '75%'}
...
Additional arguments can be passed to the panel function using the `panel_xxx` notation (e.g. `panel_ncol = 2`, `panel_labeller = 'label_both'`). For more options read the help of [`facet_wrap`](http://ggplot2.tidyverse.org/reference/facet_wrap.html) and [`facet_grid`](http://ggplot2.tidyverse.org/reference/facet_grid.html) from `ggplot2`.
