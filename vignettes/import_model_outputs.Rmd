...
The function `xpose_data()` collects all model output files and table and organizes them into an R object commonly called `xpdb` for xpose data base. xpose makes use of the [readr](http://readr.tidyverse.org) which offers the advantages of 1) rapidly import big tables and 2) automatically import compressed table (e.g. sdtab001.zip) to increase storage capacity.
...
- `special`: contains postprocessed datasets used by functions like `vpc()`
