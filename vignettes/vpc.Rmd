...
* The `stratify` options defines up to two stratifying variable to be used when computing the VPC data. The `stratify` variables can either be provided as a character vector (`stratify = c('SEX', 'MED1')`) or a formula (`stratify = SEX~MED1`) . The former will result in the use of `facet_wrap()` and the latter of `facet_grid()` when creating the plot. With "categorical" VPC the "group" variable will also be added by default.
