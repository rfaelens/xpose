...
This command will look for the **run001.lst** file in the current directory. The `xpose_data` function gather the all the output tables listed in the .lst file under $TABLE (eg. sdtab, cotab, catab, patab) as well as the run summary information as shown below. Furthermore `ggxpose` makes use of the [`readr`](https://github.com/hadley/readr) package to rapidly import big dataset into R.

``` r
$descr     : "ggxpose test run"
$run       : "run001"
$input_dat : "ggxpose_test_dataset.csv"
$nobs      : "1022"
$nind      : "74 ind."
$ofv       : "-656.873"
...
All of this information can be automatically added to the `title`, `subtitle` or `caption` of the plot (see section *Titles* below)
...
By default `title` and `subtitle` and `caption` are set to `NULL`. In this case automatic titles will be used the `title` will display the name of the plotting function and the run number for example **DV vs. IPRED |run001** the `subtitle` will show additional information such as EPS shrinkage for IPRED based plots and the `caption` will show the run directory. Templates can be provided via strings to generate informative `title`, `subtitle` and `caption`. To do so simply use one of the keywords preceded by `@` (e.g. `@ofv`) and the keyword will be automatically replaced by the value contained in the xpose\_data object when generating the plot.

``` r
dv_vs_ipred(xpdb,
            title = 'DV vs. IPRED (@run, obj: @ofv')',
            subtitle = 'Based on: @nind ind. and @nobs recs.')
```

![titles2](inst/img/titles_2.jpg)

Finally, all titles can be disabled by setting them to `FALSE`. From left to right is:
...
The argument group defines the grouping variable to be used in the dataset **for the lines only**, by default group is defined to `'ID'`. To apply a grouping variable on any other layer one can use the layer's group argument `xxx_group`, for example:
...
This command would save the graphs DV vs. IPRED under **run001\_dv\_vs\_ipred.pdf** in the current working directory. Multiple options such as manual name (`name`), the file format (`format = "pdf", "jpg" or "png"`), page size (`width`, `height`) are available.
