...
data_text <- 'Data:,   problem simtab                   data modified,     <int>  <lgl>                 <list>    <lgl>, 1       1  FALSE    <tibble [550 x 31]>    FALSE, 2       2   TRUE <tibble [11,000 x 12]>    FALSE'
files_text <- 'Files:,         name extension problem subprob method               data modified,        <chr>     <chr>   <dbl>   <dbl>  <chr>             <list>    <lgl>, 1 run001.cor       cor       1       0   foce <tibble [14 x 15]>    FALSE, 2 run001.cov       cov       1       0   foce <tibble [14 x 15]>    FALSE, 3 run001.ext       ext       1       0   foce <tibble [28 x 16]>    FALSE, 4 run001.grd       grd       1       0   foce <tibble [21 x 11]>    FALSE, 5 run001.phi       phi       1       0   foce <tibble [74 x 12]>    FALSE, 6 run001.shk       shk       1       0   foce   <tibble [7 x 5]>    FALSE'
special_text <- 'Specials:,   problem method       type        data modified,     <dbl>  <chr>      <chr>      <list>    <lgl>, 1       3    vpc continuous <list [20]>    FALSE'
# Tests start here --------------------------------------------------------
test_that('Check print.xpose_data returns a proper message', {
  expect_equal(capture_output(print(xpdb_ex_pk)), print_text)
  expect_equal(capture_output(print(xpdb_ex_pk %>% 
                                vpc_data(opt = vpc_opt(n_bins = 2)) %>% 
                                filter(ID == 110, .source = 'data') %>% 
                                filter(ITERATION == 0, .source = 'ext') %>% 
                                filter(bin == 2, .source = 'special', .where = 'vpc_dat'))), print_text_modified)
})

test_that('Check print.xpose_prm returns a proper message', {
  expect_equal(capture_output(prm_table(xpdb_ex_pk, .problem = 1)), prm_text)
})
...
test_that('Check list_data returns a proper message', {
  expect_equal(capture.output(list_data(xpdb_ex_pk)) %>% paste0(collapse = ', '), data_text)
})

test_that('Check list_files returns a proper message', {
  expect_equal(capture.output(list_files(xpdb_ex_pk)) %>% paste0(collapse = ', '), files_text)
})

test_that('Check list_special returns a proper message', {
  expect_equal(capture.output(list_special(xpdb_ex_pk %>% vpc_data(quiet = TRUE))) %>% paste0(collapse = ', '), special_text)
})
