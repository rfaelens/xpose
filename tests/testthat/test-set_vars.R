...
test_that("input is check properly", {
  expect_error(set_vars_type())
  expect_error(set_vars_units())
  expect_error(set_vars_type(xpdb_ex_pk, problem = 99), regexp = 'not found in model')
  expect_error(set_vars_label(xpdb_ex_pk, problem = 99), regexp = 'not found in model')
})

test_that("set_vars_type works properly", {
  xpdb_2 <- set_vars_type(xpdb_ex_pk, problem = NULL, 
                          idv = 'PRED', catcov = c('SS', 'II'), fake = 'HELLO')
  expect_true(is.xpdb(xpdb_2))
  expect_identical(set_vars_type(xpdb_ex_pk, problem = NULL), xpdb_ex_pk)
...
test_that("set_vars_units and set_vars_label works properly", {
  xpdb_3 <- set_vars_units(xpdb_ex_pk, problem = NULL, 
                          CL = 'L/h', V = 'L', fake = 'HELLO')
  expect_true(is.xpdb(xpdb_3))
  expect_identical(set_vars_label(xpdb_ex_pk, problem = NULL), xpdb_ex_pk)
