...
#  expect_equal(sum_out(sum_reference(model, software)), c('ref', 'na'))
  ...
  expect_equal(sum_out(sum_description(model, software)), c('descr', 'Run for testthat'))
  expect_equal(sum_out(sum_label(model, software)), c('label', 'xpose test run'))
  ...
  expect_equal(sum_out(sum_warnings(model, software)), c('warnings', '(WARNING 2) NM-TRAN INFERS THAT THE DATA ARE POPULATION.'))
