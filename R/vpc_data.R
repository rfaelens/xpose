#' Import and compute data for visual predictive plots
#'
#' @description Function designed to import and compute data for VPC. This function is a modified version of the vpc::vpc() function
#' developped by Ron Keizer \url{https://github.com/ronkeizer/vpc}.
#' 
#' @param xpdb An xpose database object.
#' @param psn_folder Optional, specify a PsN-generated VPC-folder.
...
vpc_data <- function(xpdb       = NULL,
  ...
  if (is.null(psn_folder)) {
    ...
    if (is.na(sim_problem) & is.na(obs_problem)) {
      msg('At least a simulation or an observation dataset are required to create a plot! Alternatively a `psn_folder` can be specified.', quiet)
      return()
    }
    ...
    obs_data <- read_nm_tables(files = file_path(psn_folder, 'm1', dir(paste0(psn_folder, '/m1'), pattern = 'original.npctab')[1]))
    sim_data <- read_nm_tables(files = file_path(psn_folder, 'm1', dir(paste0(psn_folder, '/m1'), pattern='simulation.1.npctab')[1]))
  } 
